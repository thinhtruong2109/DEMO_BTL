...
function TaoMonHoc() {
  ...
  const handleSuccessClose = () => {
    ...
    e.preventDefault();
    try {
      const response = await axios.get('https://school-system-nwgo.onrender.com/coursetoregistration/check');
      const { status, message } = response.data;
      if (status === 'success') {
        if (message === 'Registration is not available') {
          // Phiếu đăng kí chưa tồn tại, tiến hành tạo phiếu
          await axios.post('https://school-system-nwgo.onrender.com/courses/startForRegist');
      ...
      if (status === 'success') {
        if (message === 'Registration is not available') {
          ...
          // Phiếu đăng kí đã tồn tại
            const response_1 = await axios.get('https://school-system-nwgo.onrender.com/coursetoregistration/check-teachers');
    ...
    try {
      ...
      if (status === 'success') {
        if (message === 'Registration is not available') {
          ...
          // Phiếu đăng kí đã tồn tại
            const response_1 = await axios.get('https://school-system-nwgo.onrender.com/coursetoregistration/process-all-without-teacher');
  ...
  const handleBatDauHocKi = async(e) =>{
    e.preventDefault();
    try {
      const response = await axios.get('https://school-system-nwgo.onrender.com/courseclass/start-new-semester');
  ...
  const handleKetThucHocKi = async(e) =>{
    e.preventDefault();
    try {
      const response = await axios.post('https://school-system-nwgo.onrender.com/courseclass/endSemester');
